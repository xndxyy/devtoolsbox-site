...
export function CalculatorClient({ calculator }: Props) {
  ...
  const result = useMemo(() => calculateResult(calculator.slug, values), [calculator.slug, values])
      ...
      <section className="rounded-[28px] border border-white/10 bg-white/5 p-5 shadow-2xl shadow-black/20 sm:p-6">
        <div className="mb-6 flex items-center justify-between gap-3">
          ...
          <button
            type="button"
            onClick={() =>
              setValues(Object.fromEntries(calculator.fields.map((field) => [field.id, field.defaultValue])))
            }
            ...
            >
              ...
              <div className="mt-3 flex items-center rounded-2xl border border-white/10 bg-black/20 px-3 py-3 focus-within:border-cyan-300/40">
                ...
                <input
                  ...
                  min={field.min}
                  max={field.max}
                  value={formatNumber(values[field.id] ?? field.defaultValue, getDecimals(field.step ?? 1))}
                  onChange={(event) => {
